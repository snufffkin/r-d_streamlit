...
Ты — эксперт по оценке качества синтетических диалогов «ученик-учитель».
...
## Оцениваемая реплика:
- Реплика учителя перед ней: {teacher_before}
...
Справка по интентам:
- **answer** — ученик даёт ответ на вопрос/задание учителя. Это должен быть именно ответ, а НЕ вопрос, НЕ просто согласие, НЕ просьба.
- **question** — ученик задаёт вопрос учителю.
- **agreement** — ученик соглашается, подтверждает, принимает сказанное.
- **disagreement** — ученик не согласен, возражает.
- **clarification** — ученик просит уточнить или переспрашивает.
- **confusion** — ученик выражает непонимание, растерянность.
- **request_hint** — ученик просит подсказку.
- **correction** — ученик исправляет свой предыдущий ответ.
- **thinking_aloud** — ученик рассуждает вслух.
- **off_topic** — реплика не по теме.
- **greeting** — приветствие.
- **farewell** — прощание.

Для интента **answer** будь особенно строг: если ученик задаёт вопрос, просто соглашается или просит что-то — это НЕ answer.
